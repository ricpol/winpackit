...
__all__ = ['BOOTSTRAP_PY_SCRIPT', 'PACKIT_CONFIG_SCRIPT', 'GETPIP_URL',
           'PY_URL', 'LOG_ALWAYS', 'LOG_DEBUG', 'LOG_VERBOSE',
...
MAX_MICRO_VERSIONS = {(3, 5): 4, (3, 6): 8, (3, 7): 6, (3, 8): 1, (3, 9): 7}
MAX_MINOR_VERSIONS = {3: 9}
...
class Packit:
    def __init__(self, settings):
        ...
        self.entry_points_scripts = None  # executable in python Scripts
    ...
    def _fix_imports(self):
        ...
        with open(self.target_py_dir / pth_file, 'a') as f:
            pkgsite = str(Path('Lib') / 'site-packages')
            f.write(f'{pkgsite}\n')
            for pth in self.target_proj_dirs:
                prject = str(Path(f'../{pth.name}'))
                f.write(f'{prject}\n')
    ...
    def getprojects(self):
        ...
        self.entrypoints = [[str(p), n, f] for p, n, f in self.entry_points]
        if got_errors:
            self.msg(LOG_VERBOSE, 'ERROR: not all entry points actually exist.')
            return False
        self.msg(LOG_VERBOSE, 'Projects entry point successfully created.')
        return True

    def getscripts(self):
        """NEW: Creates bootstrap machinery for the python Scripts."""
        self.msg(LOG_VERBOSE, "\n****** Get python scripts ******")
        pathbin = 'Scripts' if sys.platform == 'win32' else 'bin'

        if not self.cfg.SCRIPTS:
            self.msg(LOG_VERBOSE, 'Skipped, no entry point scripts present.')
            return True

        got_errors = False
        self.scriptpoints = []
        for script, name, flavor in self.cfg.SCRIPTS:
            try:
                if Path(script).suffix in ('.py', '.pyw'):
                    pth = Path(self.target_py_dir) / 'Lib' / 'site-packages'
                    self.scriptpoints.append([str(Path(pth) / script),
                                              name,
                                              Path(script).suffix.split('.')[1]
                                              ])
                else:
                    pth = Path(self.target_py_dir) / pathbin / script
                    self.scriptpoints.append([str(pth), name, flavor])
            except ValueError:
                self.msg(LOG_DEBUG, '->Debug - missing items (script, name, flavor):')
                got_errors = True

        if got_errors:
            self.msg(LOG_VERBOSE, 'ERROR: incomplete entry points for scripts.')
            return False
        self.msg(LOG_VERBOSE, 'Scripts entry point successfully created.')
        return True

    def make_bootstrap(self):
        self.msg(LOG_VERBOSE, "\n****** Make bootstrap ******")

        if self.entrypoints and self.scriptpoints:
            entrypoints = self.entrypoints + self.scriptpoints

        elif not self.entrypoints:
            entrypoints = self.scriptpoints

        elif not self.scriptpoints:
    ...
    def main(self):
        ...
        retcodes.append(self.getprojects())
        retcodes.append(self.getscripts())
...
# A list of 0 or more scripts installed with pip, e.g.
# SCRIPTS = [
#    [['scriptname.exe', 'Myprog', 'flavor'], [...] )
# where *scriptname* can be the name of an executable in the Python Scripts/bin
# directory or a main launcher in the Python site-packages directory,
# which *must* be relative to site-packages directory; *Myprog* is the name you
# prefer to assign to the new link; flavor one of 'py' or 'pyw' .
# If used, this list will be included in the project list:
SCRIPTS = []
...
pack_settings = cfg(HERE, VERBOSE, USE_CACHE, PYTHON_VERSION,
                    ...
                    GOODBYE_MESSAGE, custom_action, SCRIPTS)
